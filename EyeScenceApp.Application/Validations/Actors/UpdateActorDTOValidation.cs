...
{
    public class UpdateActorDTOValidation : AbstractValidator<UpdateActorDTO>
    {
        public UpdateActorDTOValidation() { 
          
            RuleFor(actor => actor.Id)
                .NotEmpty().WithMessage("Actor ID is required.")
                .NotEqual(Guid.Empty).WithMessage("Actor ID cannot be an empty GUID.");
