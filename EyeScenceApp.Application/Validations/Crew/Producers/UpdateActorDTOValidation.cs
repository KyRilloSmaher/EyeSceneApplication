...
namespace EyeScenceApp.Application.Validations.Producers
{
    public class UpdateProducerDTOValidation : AbstractValidator<UpdateProducerDTO>
    {
        public UpdateProducerDTOValidation() { 
          
            RuleFor(Producer => Producer.Id)
                .NotEmpty().WithMessage("Producer ID is required.")
                .NotEqual(Guid.Empty).WithMessage("Producer ID cannot be an empty GUID.");
