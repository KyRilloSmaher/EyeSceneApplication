...
namespace EyeScenceApp.Application.Validations.Editors
{
    public class UpdateEditorDTOValidation : AbstractValidator<UpdateEditorDTO>
    {
        public UpdateEditorDTOValidation() { 
          
            RuleFor(Editor => Editor.Id)
                .NotEmpty().WithMessage("Editor ID is required.")
                .NotEqual(Guid.Empty).WithMessage("Editor ID cannot be an empty GUID.");
